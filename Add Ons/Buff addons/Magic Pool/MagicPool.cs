...
namespace Server.Items
{
	public class MagicPool : BaseAddon
	{
		...
		public virtual TimeSpan DecayDelay{ get{ return TimeSpan.FromMinutes( 90.0 ); } } // HOW LONG UNTIL THE POOL DECAYS IN MINUTES
		...
		public override void OnComponentUsed( AddonComponent ac, Mobile from )
		{
			...
			else if ( m_Uses > 0 )
			{
				...
				else if ( m_Pool == 4 ) // CURE
				{
					...
					{
						from.SendMessage( "Bwhahahahahaha!  You drink from the pool and nothing happens!  You dumbass!" );
						this.m_Uses = this.m_Uses - 1;
					}
				}
				else if ( m_Pool == 5 ) // HEAL
				{
					...
					{
						from.SendMessage( "Bwhahahahahaha!  You drink from the pool and now you're poisoned!" );
						this.m_Uses = this.m_Uses - 1;
					}
				}
				...
				else if ( m_Pool == 7 ) // GOLD TO LEAD
				{
					...
					if ( cont.ConsumeTotal( typeof( Gold ), m_Amount ) )
					{
						...
						from.SendMessage( "Bwhahahahahaha!  After drinking from the pool, you notice all of your gold has turned to lead!" );
						Effects.SendLocationParticles( EffectItem.Create( from.Location, from.Map, EffectItem.DefaultDuration ), 0x3728, 10, 10, 5023 );
					}
					else
					{
						from.SendMessage( "Bwhahahahahaha!  You drink from the pool and your penis falls off!" );
					}
					this.m_Uses = this.m_Uses - 1;
				}
				else if ( m_Pool == 8 ) // EQUIPPED ITEM DISAPPEARS
				{
					...
					{
						mReturn = 1;
						from.SendMessage( "Bwhahahahahaha!  You drink from the pool and nothing happens!" );
					}
					if ( mReturn != 1 )
					{
						from.SendMessage( "Bwhahahahahaha!  After drinking from the pool, you notice ALL of your equipped items disappears!" );
						Effects.SendLocationParticles( EffectItem.Create( from.Location, from.Map, EffectItem.DefaultDuration ), 0x3728, 10, 10, 5023 );
					}
				}
				else if ( m_Pool == 9 ) // LOSE A STAT POINT
				{
					...
					if ( m_Bonus > 8 )
					{
						if ( from.RawStr > 10 ) { from.RawStr = from.RawStr - 1; from.SendMessage( "Bwhahahahahaha!  You lose a strength after drinking from the pool!" ); }
						else { from.SendMessage( "Bwhahahahahaha!  You drink from the pool and nothing happens!" ); mCurse = 0; }
					}
					else if ( m_Bonus > 5 )
					{
						if ( from.RawDex > 10 ) { from.RawDex = from.RawDex - 1; from.SendMessage( "Bwhahahahahaha!  You lose a dexterity after drinking from the pool!" ); }
						else { from.SendMessage( "Bwhahahahahaha!  You drink from the pool and nothing happens!" ); mCurse = 0; }
					}
					else
					{
						if ( from.RawInt > 10 ) { from.RawInt = from.RawInt - 1; from.SendMessage( "Bwhahahahahaha!  You lose an intelligence after drinking from the pool!" ); }
						else { from.SendMessage( "Bwhahahahahaha!  You drink from the pool and nothing happens!" ); mCurse = 0; }
					}
				...
				else // POISON
				{
					...
					else 
					{
						...
						from.SendMessage( "Bwhahahahahaha!  You feel more sick after drinking from the pool!" );
						this.m_Uses = this.m_Uses - 1;
					}
				}
			}
