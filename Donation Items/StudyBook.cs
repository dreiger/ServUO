...
namespace Server.Items
{
    public class StudyBook : Item
    {
		...
		private static readonly int SkillGainInterval = 15;//This is the number of minutes that pass before a skill gain happens while studying.
		private static readonly int SkillGainedPerTick = 5;//This is the skill gain for each gain interval that has passed.
		private static readonly int SkillGainMax = 300;//This is the maximum gain that can be achieved from one study session.
		private static readonly int HoursTilAcceleratedSkillGain = 8;//This is the amount of study hours required to gain a Scroll of Alacrity effect.
		private static readonly TimeSpan AcceleratedSkillGainTime = TimeSpan.FromMinutes(60.0);//This is the duration of the accellerated skill gain.
		...
		public void EndStudy(Mobile from)
		{
			...
			if (toGain > SkillGainMax)
				toGain = SkillGainMax;
	//		if (toGain > 0)
	//			SkillCheck.Gain(from, from.Skills[this._TrainingSkill], toGain);
			else
		...
		//		from.SendMessage(0, "You may not study skills while in jail.");
			...
			else if (!this._IsInUse && this._Reader == null)
			{
				...
				from.SendMessage(0, "You are already prepared to start studying this book.");
		}
