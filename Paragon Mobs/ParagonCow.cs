...
{
    [CorpseName("a Paragon Cow corpse")]
    public class ParagonCow : BaseCreature
    {
            ...
            : base(AIType.AI_Animal, FightMode.Aggressor, 10, 1, 0.2, 0.4)
        {
            ...
            if (Core.AOS && Utility.Random(1000) == 0) // 0.1% chance to have mad ParagonCows
                this.FightMode = FightMode.Closest;
        }

        public ParagonCow(Serial serial)
            : base(serial)
        {
        }
        ...
        public bool TryMilk(Mobile from)
        {
            if (!from.InLOS(this) || !from.InRange(this.Location, 2))
                from.SendLocalizedMessage(1080400); // You can not milk the ParagonCow from this location.
            if (this.Controlled && this.ControlMaster != from)
                from.SendLocalizedMessage(1071182); // The ParagonCow nimbly escapes your attempts to milk it.
            if (this.m_Milk == 0 && this.m_MilkedOn + TimeSpan.FromDays(1) > DateTime.UtcNow)
                from.SendLocalizedMessage(1080198); // This ParagonCow can not be milked now. Please wait for some time.
            else
            {
                if (this.m_Milk == 0)
                    this.m_Milk = 4;
